...
export default function CargarEnviosHubPage() {
  ...
  const [embedTitle, setEmbedTitle] = useState("Cargar envíos")
    ...
    <div className="min-h-screen bg-[#f7f8fc]">
      ...
      <main className={`px-4 pb-6 pt-3 ${montserrat.className}`}>
        <div className="mx-auto w-full max-w-[1100px] space-y-5">
          <div>
            <h1 className="text-[34px] font-semibold tracking-tight text-[#1570ef]">Cargar envíos</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
          <div className="h-[92vh] w-[96vw] max-w-[1400px] overflow-hidden rounded-2xl border border-[#e6eaf4] bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#eef1f8] bg-[#fafbff] px-4 py-3">
              <p className="text-[14px] font-semibold text-[#1f2433]">{embedTitle}</p>
              <button
                type="button"
            ...
            <iframe title={embedTitle} src={url} className="h-full w-full" />
          </div>
        </div>
      )}
    </div>
