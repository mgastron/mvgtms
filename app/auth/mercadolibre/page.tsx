      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
        <div className="px-8 pt-10 pb-6 text-center border-b border-slate-100">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Con MVG para gestionar tus envíos desde la plataforma.
          </p>
        </div>

        <div className="px-8 py-6 space-y-5">
          <p className="text-sm text-slate-600 leading-relaxed">
            Al continuar, autorizás a <strong className="text-slate-700">MVG</strong> a usar solo los datos necesarios para crear y seguir envíos. Podés desvincular la cuenta cuando quieras.
          </p>

          <button
            onClick={handleContinuar}
            disabled={loading}
            className="w-full bg-amber-400 hover:bg-amber-500 text-slate-900 font-semibold py-3.5 px-4 rounded-xl transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
